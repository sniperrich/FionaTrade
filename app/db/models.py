...
class EntryPlan(Base):
    """Persisted delayed-entry plans generated by the portfolio decision."""

    __tablename__ = "entry_plans"
    __table_args__ = (
        Index("ix_entry_plans_ticker_status_updated", "ticker", "status", "updated_at"),
        Index("ix_entry_plans_status_valid_until", "status", "valid_until"),
    )

    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    ticker: Mapped[str] = mapped_column(String(16), index=True)
    agent_run_id: Mapped[int | None] = mapped_column(Integer, nullable=True, index=True)
    status: Mapped[str] = mapped_column(String(24), default="ACTIVE", index=True)  # ACTIVE/TRIGGERED/EXPIRED/CANCELLED/REPLACED/INVALIDATED
    execution_mode: Mapped[str] = mapped_column(String(48), default="WAIT_PULLBACK")
    planned_action: Mapped[str] = mapped_column(String(16), default="HOLD")  # BUY/SHORT/SELL/HOLD
